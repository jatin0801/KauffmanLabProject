...
def sample_list(request):
    ...
    # pagination
    p = Paginator(sample_filter.qs, 10) 
    ...
    return render(request, 'KauffmanLabApp/sample_list.html', {'filter': sample_filter, 'table': table, 'samples': samples, 'num_pgs':num_pgs, 'query_params': query_string})

def sample_csv(request):
    # TODO: create a session to save and export samples selected on multiple pages
    sample_filter = SampleFilter(request.GET, queryset=Sample.objects.all())
    ...
    if request.GET:
        ...
        if selections:
            selected_samples = Sample.objects.filter(Sample_ID__in=selections)
            for s in selected_samples.values_list('Sample_ID', 'LabNB_PgNo', 'Label_Note', 'Sample_Type__Sample_Type', 'Material_Type', 'Parent_Name', 'Source_ID', 'User_ID__User_Short', 'DigitalNB_Ref', 'Original_Label', 'Comments'):
                writer.writerow(s)
        else:
            filtered_samples = sample_filter.qs.values_list('Sample_ID', 'LabNB_PgNo', 'Label_Note', 'Sample_Type__Sample_Type', 'Material_Type', 'Parent_Name', 'Source_ID', 'User_ID__User_Short', 'DigitalNB_Ref', 'Original_Label', 'Comments')
            for s in filtered_samples:
                writer.writerow(s)
    else:
        samples = Sample.objects.all().values_list('Sample_ID', 'LabNB_PgNo', 'Label_Note', 'Sample_Type__Sample_Type', 'Material_Type', 'Parent_Name', 'Source_ID', 'User_ID__User_Short', 'DigitalNB_Ref', 'Original_Label', 'Comments')
        for s in samples:
            writer.writerow(s)
